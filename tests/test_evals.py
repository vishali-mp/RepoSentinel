...
class TestMatchesExpected(unittest.TestCase):
    ...
    def test_line_range_within_proximity_passes(self):
        expected = {**EXPECTED_SQL, "line_range": [2, 2]}
        f = make_finding(line_start=5, line_end=5)
        self.assertTrue(_matches_expected(f, expected, line_proximity=15))

    def test_line_range_outside_proximity_fails(self):
        expected = {**EXPECTED_SQL, "line_range": [2, 2]}
        f = make_finding(line_start=100, line_end=100)
        self.assertFalse(_matches_expected(f, expected, line_proximity=15))

    def test_line_range_absent_skips_proximity_check(self):
        expected = {"category": "security", "keywords": ["sql injection"]}
        f = make_finding(line_start=999, line_end=999)
        self.assertTrue(_matches_expected(f, expected))

    def test_line_range_end_within_proximity_passes(self):
        expected = {**EXPECTED_SQL, "line_range": [10, 15]}
        f = make_finding(line_start=28, line_end=29)
        self.assertTrue(_matches_expected(f, expected, line_proximity=15))

    def test_empty_line_range_skips_check(self):
        expected = {**EXPECTED_SQL, "line_range": []}
        f = make_finding(line_start=999, line_end=999)
        self.assertTrue(_matches_expected(f, expected))
